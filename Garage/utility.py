...
class Database():
    def __init__(self,SERVER: str,DATABASE: str) -> None:
        ...
        self._ConnectionStatement = makeConnectionStatement(self._SERVER,self._DATABASE)
...
def fetchServiceData(VehicleNo: str):
    db = create_DB()
    db.cursor.execute("select * from Services where vehicleNo=? Order by ServiceID DESC",VehicleNo)
    if db.cursor:
        return db.cursor
    else: return False
...
def updateVehileData(vehicalDict: dict):
    try:
        ...
        db.cursor.execute("Update vehicles set brand=? ,model=? where VehicleNo=?",brand,model,Vehicle_NO)
        db.cursor.commit()
        db.close()
        return True
    except:
...
def updateServiceData(serviceDict: dict):
    try:
        ...
        db.cursor.execute("update services set ServiceType =? where ServiceID=?",service_type,service_id)
        db.cursor.commit()
        db.close
        return True
    except:
...
def addVehical(vehicleDict:dict):
    try:
        ...
        db = create_DB()
        db.cursor.execute("insert into vehicles(vehicleNo,Brand,Model) Values(?,?,?)",vehicle_no,brand,model)
        db.cursor.commit()
        db.close()
        return True
    except:
